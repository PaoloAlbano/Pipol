...
function PiPVideo({ stream, label, muted = false }) {
  ...
  return (
    <div style={{ position: 'relative', flex: '1 1 0', minWidth: 0, background: '#1a1a1a', borderRadius: 6, overflow: 'hidden' }}>
      {stream ? (
      ...
      <span style={{ position: 'absolute', bottom: 4, left: 6, fontSize: 10, color: '#fff', background: 'rgba(0,0,0,0.6)', padding: '1px 5px', borderRadius: 3, maxWidth: '80%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {label}
      </span>
    </div>
  )
...
export default function PiPOverlay({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 8, height: '100%', boxSizing: 'border-box' }}>
        ...
        {remoteEntries.length === 0 ? (
          ...
          remoteEntries.map(([id, stream]) => (
            <PiPVideo
              key={id}
              stream={stream}
              label={peerById[id]?.username ?? id.slice(0, 8)}
            />
          ))
        )}
      </div>
